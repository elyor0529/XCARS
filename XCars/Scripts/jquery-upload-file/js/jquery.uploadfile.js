...
(function ($) {
    ...
    $.fn.uploadFile = function (options) {
        ...
        var s = $.extend({
            ...
            dragDropContainerClass: "dropZone",
        ...
        (function checkAjaxFormLoaded() {
            if($.fn.ajaxForm) {
                ...
                if(s.dragDrop) {
                    var dragDrop = $('<label class="' + s.dragDropContainerClass + '" style="vertical-align:top;"></label>');
                    $(obj).append(dragDrop);
                ...
                $(obj).append(obj.errorLog);
   				...
   				if(s.showQueueDiv)
		        	obj.container =$("#"+s.showQueueDiv);
   				    ...
   				    obj.container = $("<div class='add_cars_photoprev'></div>").insertAfter($(obj));
	...
	this.enqueueFile = function(file){
		...
		this.createProgress = function (filename, filepath, filesize, isMain) {
		    //console.log(filename);
		    var existing = true;
		    var pd = new createProgressDiv(this, s, filename, existing, isMain);
            pd.progressDiv.show();
            ...
            if(s.showPreview)
            {
                ...
                pd.preview.attr('style', "background-image:url(" + filepath + ")");
                //obj.attr('style', "background:url(" + e.target.result + ")");
                pd.preview.show();
            }
		...
		function noserializeAndUploadFiles(s, obj, files) {
		    var ts = $.extend({}, s);
                ...
                ts.fileData = fd;
                var pd = new createProgressDiv(obj, s, 416);
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for(var i = 0; i < files.length; i++) {
                ...
                var pd = new createProgressDiv(obj, s, 463);
                var fileNameStr = "";
        ...
        function getSrcToPreview(file, obj) {
            if (file) {
                ...
                reader.onload = function (e) {
                    ...
                    obj.attr('style', "background-image:url(" + e.target.result + ")");
                };
        ...
        function createCustomInputFile (obj, group, s, uploadLabel) {
            ...
            var form = $("<form la='asdf' method='" + s.method + "' action='" + s.url + "' enctype='" + s.enctype + "'></form>");
            ...
            fileInput.change(function () {
                ...
                if(s.serialize && feature.fileapi && feature.formdata) //use HTML5 support and split file submission
                {
                    ...
                    var pd = new createProgressDiv(obj, s, 613);
        ...
        function defaultProgressBar(obj, s, id, existing, isMain)
        {
            //alert("default id: " + id);
            var existingBar = "";
            if (existing == true)
                existingBar = " existingBar";
            ...
            this.statusbar = $("<a id='" + id + "' class='photo_prev" + existingBar + active + "' onclick='MakePhotoMain(" + id + ")'></a>")/*.width(s.statusBarWidth)*/;
            //this.preview = $("<img height='76' width='76'/>")/*.width(s.previewWidth).height(s.previewHeight)*/.appendTo(this.statusbar).hide();
            this.preview = $("<div></div>")/*.width(s.previewWidth).height(s.previewHeight)*/.appendTo(this.statusbar).hide();
            this.filename = $("").width(0).appendTo(this.statusbar);
            //this.progressDiv = $("<div class='ajax-file-upload-progress'>").appendTo(this.statusbar).hide();
            this.progressDiv = $("").appendTo(this.statusbar).hide();
            //this.progressbar = $("<div class='ajax-file-upload-bar'></div>").appendTo(this.progressDiv);
            this.progressbar = $("").appendTo(this.progressDiv);
            //this.abort = $("<div>" + s.abortStr + "</div>").appendTo(this.statusbar).hide();
            this.abort = $("").appendTo(this.statusbar).hide();
            //this.cancel = $("<div>" + s.cancelStr + "</div>").appendTo(this.statusbar).hide();
            this.cancel = $("").appendTo(this.statusbar).hide();
            //this.done = $("<div>" + s.doneStr + "</div>").appendTo(this.statusbar).hide();
            this.done = $("").appendTo(this.statusbar).hide();
            //this.download = $("<div>" + s.downloadStr + "</div>").appendTo(this.statusbar).hide();
            this.download = $("").appendTo(this.statusbar).hide();
            this.del = $("<i class='icon-cancel'>" + /*s.deleteStr +*/ "</i>").appendTo(this.statusbar).hide();
        ...
        function createProgressDiv(obj, s, id, existing, isMain) {
            //alert("create id: " + id);
	        var bar = null;
        	if(s.customProgressBar)
        		bar =  new s.customProgressBar(obj,s);
        	else
        	    bar = new defaultProgressBar(obj, s, id, existing, isMain);
            ...
            //var mainDiv = "<div class='photoIsMain' isMain='";
            //if (isMain == "True")
            //    mainDiv += "true'><div class='alert alert-info'>Main</div>";
            //else if (isMain == "False")
            //    mainDiv += "false'><button type='button' class='btn btn-primary' onclick='MakePhotoMain(" + id + ")'>Make main</button>";
            //else
            //    mainDiv += "false'>";
            //mainDiv += "</div>";

            //bar.extraHTML = $("<div class='extrahtml'>" + mainDiv + "</div>").insertAfter(bar.filename);
